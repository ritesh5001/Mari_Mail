...
export function VesselAddToListModal({ vesselIds, onClose, onDone }: Props) {
  ...
  async function handleAdd() {
    ...
    setError(null);
    try {
      const r = await apiFetch(`/api/lists/${selectedId}/vessels`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ vesselIds }),
      });
      const payload = (await r.json()) as { data?: { added?: number }; error?: { message?: string } };
      if (!r.ok) throw new Error(payload.error?.message ?? "Failed to add vessels");
      const listName = lists.find((l) => l.id === selectedId)?.name ?? "list";
      onDone(listName, payload.data?.added ?? vesselIds.length, selectedId);
    } catch (err) {
      setError((err as Error).message);
    } finally {
      setSubmitting(false);
      ...
      >
        ...
        <div className="px-5 py-4 space-y-3">
          ...
          {loading ? (
            ...
            <p className="text-center text-sm text-slate-400 py-4">No lists yet — create one above.</p>
          ) : (
            <ul className="max-h-52 overflow-y-auto divide-y divide-slate-100 rounded-md border border-slate-200">
              {lists.map((list) => (
                <li key={list.id}>
                  <button
